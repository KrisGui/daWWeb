...
class Transport extends Component {
  constructor() {
    ...
    Tone.Transport.scheduleRepeat(time => {
      Tone.Draw.schedule(() => {
        this.setState({
          metronomeAnimation: this.metronomeFrames[this.metronomeFrame % this.metronomeFrames.length]
        })
        this.metronomeFrame++
      }, time)
    }, '4n')
  ...
  setBPM = newBPM => {
    ...
    this.setState({bpm: (Math.round(Tone.Transport.bpm.value * 100) / 100).toFixed(2)})
  }
  ...
  tapBPM = e => {
    ...
    if (this.taps.length > 1) {
      ...
      tappedBPM = 60 / ((tapsTransformed.reduce((a, c) => a + c, 0)) / tapsTransformed.length)
      ...
      <div className='Transport container'>
        ...
        <div className='tempo controller'>
          <button type='button'
                  ...
                  onMouseLeave={this.clearActiveClass}> Tap </button>

          <button type='button'
                  id='decrement'
                  onMouseDown={this.adjustBPM}
                  onMouseUp={this.clearAdjustTimers}
                  onMouseLeave={this.clearAdjustTimers}> - </button>
          ...
          <button type='button'
                  id='metronome'
                  onClick={this.toggleMetronome}> {this.state.metronomeAnimation} </button>
        </div>
        ...
        <div className='playback controller'>
          <button type='button'
                  id='play-pause'
                  className=''
                  onClick={this.toggleTransport}> ❙▶ </button>

          <button type='button'
                  id='stop'
                  className='active'
                  onClick={this.stopTransport}> ■ </button>
        </div>
